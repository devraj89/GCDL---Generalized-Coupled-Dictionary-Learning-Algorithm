function [Wx,Wy,r] = cluster_cca_mod3(train_a,train_b,a_labels,b_labels,kapa_cca,knn,eta,option)
...
for k=1:size(score_mean_a,1)
    ...
    knn_index_a_mean(k,:) = sortIndex';
end
for k=1:size(score_mean_b,1)
    finalScore = score_mean_b(k,:);
    [~,sortIndex] = sort(finalScore);
    knn_index_b_mean(k,:) = sortIndex';
end
...
for c=1:C %for each class
    ...
    %find those vectors in set a having that label
    [~,idx1]=find(a_labels==unq_a_label(c));
    %find those vectors in set b not having that label
    [~,idx2]=find(b_labels~=unq_b_label(c));
    idx2 = idx2(1:knn);
    for j=1:length(idx1)
        x = train_a(:,idx1(j));
...
for c=1:C %for each class
    ...
    zz = train_a_mean(:,knn_index_a_mean(c,:));
    zz = zz(:,2:knn+1);
    sum2 = zz*zz.';
    if option==0
        sum2 = 0;
...
for c=1:C %for each class
    ...
    zz = train_b_mean(:,knn_index_b_mean(c,:));
    zz = zz(:,2:knn+1);
    sum2 = zz*zz.';
    if option==0
        sum2 = 0;
